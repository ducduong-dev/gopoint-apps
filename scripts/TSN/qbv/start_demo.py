...
device = subprocess.check_output("hostname", shell=True)

os.system("python3 /root/gopoint-apps/scripts/TSN/qbv/loading_window.py launch &")
# check if device is i.MX8mmevk
if b"8mm" in device:
    os.system("sh /root/gopoint-apps/scripts/TSN/qbv/IP_mini.sh")
    time.sleep(1)
    os.system("python3 /root/gopoint-apps/scripts/TSN/qbv/demo_qbv.py")

# check if device is i.MX8MPevk
elif b"8mp" in device:
    os.system("sh /root/gopoint-apps/scripts/TSN/qbv/IP_plus.sh")
    time.sleep(1)
