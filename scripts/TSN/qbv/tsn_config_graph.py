...
class ButtonWindow(Gtk.Window):
    ...
    def on_combobox_changed(self, combobox):
        ...
        if model[treeiter][0] == "No qbv (No Prioritization)":
            os.system(
                "python3 /root/gopoint-apps/scripts/TSN/qbv/tsnqbv.py no_qbv root 192.168.0.1 &"
            )
        ...
        if model[treeiter][0] == "Qbv1 (Video Prioritization)":
            os.system(
                "python3 /root/gopoint-apps/scripts/TSN/qbv/tsnqbv.py qbv1 root 192.168.0.1 &"
            )
            dialog = Gtk.MessageDialog(
                transient_for=self,
                flags=0,
        ...
        if model[treeiter][0] == "Qbv2 (Iperf Prioritization)":
            os.system(
                "python3 /root/gopoint-apps/scripts/TSN/qbv/tsnqbv.py qbv2 root 192.168.0.1 &"
            )
            dialog = Gtk.MessageDialog(
                transient_for=self,
                flags=0,
...
def animate(i, x_axis: list, y1_axis: list, y2_axis: list):
    try:
        ...
        x_axis.append(_Time)
        iperf_result = open(
            "/root/gopoint-apps/scripts/TSN/qbv/iperf.txt", "r"
        ).readlines()
        ...
        if "received" in iperf_result:
            ...
            iperf_result1 = open("/root/gopoint-apps/scripts/TSN/qbv/iperf1.txt", "w")
            iperf_result1.write(iperf_result)
            iperf_result1.close()
        iperf = open("/root/gopoint-apps/scripts/TSN/qbv/iperf1.txt", "r").readlines()
        iperf = iperf[0].strip()
        iperf = float(iperf)
        FPS = open("/root/gopoint-apps/scripts/TSN/qbv/FPS.txt", "r").readlines()
        FPS = FPS[0].strip()
        FPS = float(FPS)
