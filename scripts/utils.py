...
DOWNLOAD_FOLDER = "/root/gopoint-apps/downloads"
DOWNLOAD_DB = "/root/gopoint-apps/downloads.json"
...
def download_file(file_name):
    ...
    if exists(DOWNLOAD_FOLDER + "/" + file_name):
        ...
        out = subprocess.getstatusoutput(
            "wget -O  /root/gopoint-apps/downloads/" + file_name + " " + url
        )[0]
        if out != 0:
            out = subprocess.getstatusoutput(
                "wget -O  /root/gopoint-apps/downloads/" + file_name + " " + alt_url
            )[0]
        if out != 0:
            return -2
        path = DOWNLOAD_FOLDER + "/" + file_name
