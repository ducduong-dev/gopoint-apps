...
class DMSDemo:
    ...
    def __init__(self, video_device, inf_device, model_path, resolution):
        """
        ...
        if self.platform == "i.MX95":
            videoconvert = "imxvideoconvert_g2d ! "
            videocrop = "videocrop top=0 left=80 right=80 bottom=0 ! "
            info_image = "/imx95_dms_info.jpeg"
            compositor = "imxcompositor_g2d "

        display_resolution = resolution.split("x")
        cam_pipeline = (
            "v4l2src device="
            + video_device
            ...
            + "sink_0::xpos=840 sink_0::ypos=0 sink_0::height=1080 sink_0::width=1080 ! "
            + "cairooverlay name=drawer ! "
            + "queue max-size-buffers=2 leaky=2 ! waylandsink "
            + "window-width=" + display_resolution[0]
            + " window-height=" + display_resolution[1]
        ...
        if self.platform == "i.MX95" and inf_device == "NPU":
            face_model = model_path + "/face_detection_ptq_neutron.tflite"
            landmark_model = model_path + "/face_landmark_ptq_neutron.tflite"
            iris_model = model_path + "/iris_landmark_ptq_neutron.tflite"
            smk_call_model = model_path + "/yolov4_tiny_smk_call_neutron.tflite"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--resolution", type=str, default="1920x1080", help="Resolution of display"
    )
    args = parser.parse_args()
    Gst.init(None)
    window = DMSDemo(args.device, args.backend, args.model_path, args.resolution)
    while True:
