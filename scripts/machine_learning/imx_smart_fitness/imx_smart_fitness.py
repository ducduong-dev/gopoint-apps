...
class ImxSmartFitness:
    ...
    def start(self, widget):
        ...
        self.output_process = subprocess.Popen(
            self.cache_enable + " /root/gopoint-apps/scripts/"
            "machine_learning/imx_smart_fitness/imx-smart-fitness "
            "--device="
            + device
            + " --target="
            + self.platform
            + " --pose-detection-model=/root/gopoint-apps/downloads/"
            + (
                self.model_detection_tflite
                if self.platform == "i.MX8MP"
                else self.model_detection_vela
            )
            + " --pose-landmark-model=/root/gopoint-apps/downloads/"
            + (
                self.model_landmark_tflite
                if self.platform == "i.MX8MP"
                else self.model_landmark_vela
            )
            + " --pose-embeddings=/root/gopoint-apps/downloads/pose_embeddings.csv"
            " --anchors=/root/gopoint-apps/downloads/anchors.txt",
            shell=True,
