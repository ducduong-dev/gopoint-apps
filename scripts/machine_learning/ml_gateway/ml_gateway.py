...
class ServerWindow:
    ...
    def __init__(self):
        ...
        glade_file = (
            "/root/gopoint-apps/scripts/machine_learning/ml_gateway/ml_gateway.glade"
        )
    ...
    def preload(self):
        ...
        if self.platform == "i.MX93":
            self.compile_vela()
        elif self.platform == "i.MX8MP":
            os.environ["VIV_VX_CACHE_BINARY_GRAPH_DIR"] = "/root/gopoint-apps/downloads"
            os.environ["VIV_VX_ENABLE_CACHE_GRAPH_BINARY"] = "1"

            if not any(
                f.endswith(".nb") for f in os.listdir("/root/gopoint-apps/downloads")
            ):
    ...
    def to_reduce_warmup(self):
        """To reduce NPU warmup time on i.MX 8M Plus"""
    ...
    def compile_vela(self):
        ...
        if not os.path.exists(self.vela_model):
            ...
            subprocess.run(
                "vela " + self.model + " --output-dir=/root/gopoint-apps/downloads/",
                shell=True,
                check=True,
            )
...
class ClientWindow:
    ...
    def connect_to_server(self, widget):
        ...
        client_pipeline += " mix.sink_0 t. ! queue max-size-buffers=2 ! mix.sink_1"
        client_pipeline += (
...
if __name__ == "__main__":
    ...
    if PLATFORM in ("i.MX8MP", "i.MX93", "i.MX95"):
