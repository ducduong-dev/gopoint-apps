...
This script launches the Pose Estimation NNStreamer example using a UI to pick settings.
"""
...
class NNStreamerLauncher:
    """The GUI window for the Pose Estimation example launcher"""
    ...
    def __init__(self):
        ...
        glade_file = (
            "/root/gopoint-apps/"
            "scripts/machine_learning/nnstreamer/pose_estimation/pose_estimation.glade"
        )
        ...
        # General variables
        self.video_file = "Conditioning_Drill_1-_Power_Jump.webm.480p.vp9.webm"
        self.tflite_model = "movenet_single_pose_lightning.tflite"
        self.npu_tflite_model = "movenet_quant.tflite"
        self.vela_tflite_model = MODELS_PATH + "movenet_quant_vela.tflite"
        ...
        devices = []
        if self.platform in ("i.MX8MP"):
            self.sources_list.append_text("Example video")
        for device in glob.glob("/dev/video*"):
            self.sources_list.append_text(device)
            devices.append(device)
        self.sources_list.set_active(0)

        if self.platform in ("i.MX8MM", "i.MX8QM") and "/dev/video3" in devices:
        ...
        resolutions = ["1920x1080@30", "640x480@30"]
    ...
    def preload(self):
        ...
        GLib.idle_add(self.status_bar.set_text, "Downloading video file...")
        self.video_file = utils.download_file(self.video_file)

        # Verify if download is successfull
        if self.video_file == -1:
            GLib.idle_add(
                self.status_bar.set_text,
                "Cannot find files!\n"
                "Make sure required files are available in downloads database!",
        ...
        GLib.idle_add(self.status_bar.set_text, "Video file successfully downloaded!")
    ...
    def start(self, widget):
        ...
        if device == "Example video":
            uses_camera = 0
            source = " -f " + self.video_file
        else:
            source = " --camera_device " + device
            uses_camera = 1
        ...
        if self.resolution_list.get_active_text() == "1920x1080@30":
            res = " -r 1920,1080,30 "
        else:
            res = " -r 640,480,30 "
        ...
        pipeline = (
            "/root/gopoint-apps/scripts/machine_learning/nnstreamer/pose_estimation/example_pose_movenet_tflite"
            + cam_op
            + " -b "
            + backend
            + normalization
